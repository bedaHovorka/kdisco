...
class PilotEjectionTest {
     ...
     *
     * Two separate @Test methods calling simulateEjection() twice share the same JVM
     * (forkEvery=1 isolates by class, not by method), so jDisco global Variable state
     * from the first run bleeds into the second.  One test, one run avoids that.
     */
    @Test
    @Timeout(30, unit = TimeUnit.SECONDS)
    fun `ejection is safe at slow speed and Phase2 derivatives execute`() {
    ...
    private fun simulateEjection(vA: Double, h: Double): EjectionResult {
        ...
        class PilotEjection : Process() {
            override fun actions() {
                ...
                // after runSimulation returns would give the t=endTime value, not the
